...
#[embassy::task]
async fn agc_cpu_task(
    mut downrupt: DownruptPeriph<'static>,
    mut dsky: DskyDisplay<'static>)
{
    let mut q = Queue::new();
    let (rupt_tx, _rupt_rx) = q.split();
...
pub fn init_agc_tasks(spawner: &Spawner) {
    ...
    let (kp_tx, kp_rx) = keypress_queue.split();
    ...
    let _ = spawner.spawn(agc_cpu_task(downrupt, dsky));
}
